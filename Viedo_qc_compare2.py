...
def compareVideo(dir_reference, dir_save, mask_images, threshold):
    ...
    while True:
        ...
        for index, mask_image in enumerate(mask_images):
            ...
            try:
                ...
                img_ssim = pytorch_ssim.ssim(img1_crop, img2_crop).item()
                ...
                for j in coordinate[index]:
                    ...
                    img1_crop = Variable(img1_crop, requires_grad=False)
                    img2_crop = Variable(img2_crop, requires_grad=False)
                    ssim = pytorch_ssim.ssim(img1_crop, img2_crop).item()
                    ssim_avg.append(ssim)
                img_ssim = statistics.mean(ssim_avg)
            ...
            if img_ssim > threshold:
                ...
                if start_flag and statistics.mean(previous_ssim) < threshold:
                    # close viedo writer
                    # segment count ++
                ...
                start_flag = True
            previous_ssim.append(img_ssim)
            previous_ssim.popleft()
            pbar.update(fi)
            dosomework()
        if start_flag:
    ...
    df = pd.DataFrame(l, columns=['Time', 'image', 'SSIM Value'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--threshold', type=float, help='SSIM threshold', default=0.8)
